...
public class MailServiceImpl implements MailService {
	...
	@Value("${mail.content.reset.password:}")
	...
	public void sendPasswordResetEmail(Object object, String temp) {
		...
		MimeMessagePreparator preparator = getMessagePreparator(user, mailFrom, dear, contentResetPassword,
				subjectResetPassword, link);

		log.debug("dear={},content={},subject={},mailForm={}", dear, contentResetPassword,
				subjectResetPassword, mailFrom);

		try {
			mailSender.send(preparator);
			log.debug("User({})'s password reset email has been sent.", user.getUsername());
		} catch (MailException ex) {
			log.debug("User({})'s password reset email not be sent.", user.getUsername());
		}
